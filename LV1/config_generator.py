...
class ConfigGenerator:
    ...
    def find_available_blocks(self):
        ...
        if not voxel_cache_dir.exists():
            print(f"  [ERROR] voxel_cache 폴더가 존재하지 않습니다: {voxel_cache_dir}")
            return available_blocks

        # voxel_cache에서 JSON 파일들 수집
        json_files = list(voxel_cache_dir.glob("*.json"))

        if not json_files:
            print(f"  [ERROR] voxel_cache 폴더에 블록 데이터가 없습니다!")
            return available_blocks

        # 블록 이름과 경로 수집
        for json_file in sorted(json_files):
            block_name = json_file.stem
            available_blocks.append((block_name, str(json_file)))

        print(f"  [INFO] voxel_cache에서 {len(available_blocks)}개 블록 발견")
        return available_blocks
    ...
    def load_from_voxel_cache(self, block_name):
        ...
        current_dir = Path(__file__).parent
        project_root = current_dir.parent
        cache_file = project_root / "voxel_cache" / f"{block_name}.json"
        
        print(f"  [DEBUG] 캐시 파일 경로: {cache_file}")
        print(f"  [DEBUG] 파일 존재 여부: {cache_file.exists()}")
        ...
        if cache_file.exists():
            try:
                ...
                print(f"  [DEBUG] JSON 로드 성공, 키들: {list(cache_data.keys())}")
                
                if 'voxel_data' not in cache_data:
                    print(f"  [ERROR] voxel_data 키가 없습니다")
                    return None
                ...
                print(f"  [DEBUG] voxel_data 키들: {list(voxel_data.keys()) if isinstance(voxel_data, dict) else 'voxel_data is not dict'}")
            ...
            print(f"  [ERROR] 캐시 파일이 존재하지 않습니다: {cache_file}")
        return None
