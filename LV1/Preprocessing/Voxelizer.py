...
class OptimizedVoxelizer:
    ...
    def _calculate_surface_flatness(self, voxels_3d, transform):
        """개선된 표면 평탄도 계산 - 돌출부 감지 강화"""
        # 변환에 따라 복셀 데이터 회전/반전
        if transform == 'flip_z':
            surface_voxels = voxels_3d[:, :, ::-1]
        elif transform == 'rotate_y':
            surface_voxels = np.transpose(voxels_3d, (2, 1, 0))
        elif transform == 'rotate_y_flip':
            surface_voxels = np.transpose(voxels_3d, (2, 1, 0))[::-1, :, :]
        elif transform == 'rotate_x':
            surface_voxels = np.transpose(voxels_3d, (0, 2, 1))
        elif transform == 'rotate_x_flip':
            surface_voxels = np.transpose(voxels_3d, (0, 2, 1))[:, ::-1, :]
        else:
            surface_voxels = voxels_3d

        x_size, y_size, z_size = surface_voxels.shape

        # 1단계: 각 (x,y) 위치에서 바닥면 높이 수집
        bottom_heights = []
        coverage_map = np.zeros((x_size, y_size), dtype=bool)

        for x in range(x_size):
            for y in range(y_size):
                for z in range(z_size):
                    if surface_voxels[x, y, z]:
                        bottom_heights.append(z)
                        coverage_map[x, y] = True
                        break

        if len(bottom_heights) < 5:
            return 0.0

        # 2단계: 주요 바닥면 레벨 찾기 (가장 많은 복셀이 있는 높이)
        height_counts = {}
        for h in bottom_heights:
            height_counts[h] = height_counts.get(h, 0) + 1

        main_floor_height = max(height_counts.keys(), key=lambda k: height_counts[k])
        main_floor_count = height_counts[main_floor_height]
        total_coverage = len(bottom_heights)

        # 3단계: 주요 바닥면 비율 계산
        main_floor_ratio = main_floor_count / total_coverage

        # 4단계: 높이 편차 분석
        z_range = max(bottom_heights) - min(bottom_heights)
        z_std = np.std(bottom_heights)

        # 5단계: 돌출부 감지 - 주요 바닥면보다 높은 부분들
        protrusion_count = sum(1 for h in bottom_heights if h > main_floor_height + 1)
        protrusion_ratio = protrusion_count / total_coverage

        # 6단계: 종합 평탄도 점수 계산
        # 주요 바닥면 비율이 높을수록 평탄함
        main_floor_score = main_floor_ratio
        ...
        if z_range <= 1:
            ...
            height_score = max(0.2, 1.0 / (1.0 + z_range * 0.2))

        # 돌출부가 적을수록 평탄함
        protrusion_penalty = 1.0 - protrusion_ratio * 0.8

        # 최종 평탄도 = 주요면 비율 × 높이 점수 × 돌출부 페널티
        final_flatness = main_floor_score * height_score * protrusion_penalty

        return min(1.0, max(0.0, final_flatness))

    def _get_projection_area(self, voxels_3d, transform):
        """2D 투영 면적 계산 - 위에서 내려다본 면적 (울퉁불퉁함 무관)"""
        # 변환 적용
        if transform == 'flip_z':
            surface_voxels = voxels_3d[:, :, ::-1]
        elif transform == 'rotate_y':
        ...
        # 2D 투영 면적: Z축 방향에서 하나라도 복셀이 있으면 카운트
        projection = np.any(surface_voxels, axis=2)
        return np.sum(projection)

    def optimize_block_orientation(self, voxels_3d, bbox):
        """
        [AUTO] 바닥 접촉면을 최대화하는 자동 방향 최적화 (예전 방식 적용)
    ...
    def voxelize_improved(self, mesh, resolution):
        """간단하고 신뢰성 있는 복셀화 - Trimesh 내장 + 자동 방향 최적화"""
        print("  - Starting Trimesh built-in voxelization with orientation optimization...")
        print(f"    Using fixed resolution: {resolution}m (ship block optimized)")
        
        # Trimesh 내장 복셀화 (단일 방법)
        try:
            print("    Trimesh built-in voxelization")
            voxelized_mesh = mesh.voxelized(pitch=resolution)
            voxels = voxelized_mesh.matrix
            bbox = voxelized_mesh.bounds
            
            print(f"    - Voxelization result: {np.sum(voxels):,} voxels")
            
            if np.sum(voxels) > 0:
                print("    - SUCCESS: Built-in voxelization completed")
                
                # [AUTO] 자동 방향 최적화 적용
                voxels_optimized, bbox_optimized, orientation = self.optimize_block_orientation(voxels, bbox)
            ...
            print(f"    - 메시 파일이 손상되었거나 watertight하지 않을 수 있습니다")
            print(f"    - 메시 복구 또는 다른 해상도 시도를 권장합니다")
            raise e
...
class ImprovedVisualizer:
    ...
    def visualize_improved_comparison(self, voxels_3d, voxel_data_25d_list, bbox, resolution, block_id):
        ...
        fig = plt.figure(figsize=(24, 12))  # 높이 줄임 (16 → 12)
        ...
        # 1행: 3D 원본 (여러 각도)
        ax_3d_1 = plt.subplot2grid((3, num_methods + 1), (0, 0), projection='3d')  # 4 → 3행으로 변경
        ...
        ax_3d_2 = plt.subplot2grid((3, num_methods + 1), (0, 1), projection='3d')
        ...
        for i, result_dict in enumerate(voxel_data_25d_list):
            if i < num_methods - 1:  # 공간이 있는 경우만
                ...
                ax_25d_top = plt.subplot2grid((3, num_methods + 1), (0, i + 2))
        ...
        for i, result_dict in enumerate(voxel_data_25d_list):
            method_name = result_dict['method']
            voxel_data_25d = result_dict['voxel_data']
            ax_25d_3d = plt.subplot2grid((3, num_methods + 1), (1, i), projection='3d')
            self.render_25d_3d_view_improved(ax_25d_3d, voxel_data_25d, bbox, resolution)
            ax_25d_3d.set_title(f'2.5D {method_name}\n(3D View)', fontsize=10)
        
        # 3행: 정확도 분석 (외곽선 비교 제거)
        ax_accuracy = plt.subplot2grid((3, num_methods + 1), (2, 0), colspan=num_methods + 1)
