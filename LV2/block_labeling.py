...
class BlockLabeler:
    def __init__(self,
                voxel_cache_dir: str = None,
                vessel_specs_file: str = None,
                safety_margin: float = 2.0):
        # 절대 경로로 기본값 설정
        if voxel_cache_dir is None:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            project_root = os.path.dirname(script_dir)
            voxel_cache_dir = os.path.join(project_root, "voxel_cache")
        
        if vessel_specs_file is None:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            project_root = os.path.dirname(script_dir)
            vessel_specs_file = os.path.join(project_root, "vessel_specs.json")
    ...
    def _load_vessel_specs(self) -> List[Dict]:
        if not self.vessel_specs_file.exists():
            raise FileNotFoundError(f"vessel_specs.json 파일이 없습니다: {self.vessel_specs_file}")
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                raise ValueError("vessel_specs.json 형식이 올바르지 않습니다.")
        except Exception as e:
            raise Exception(f"vessel_specs.json 읽기 실패: {e}")
...
def main():
    ...
    ap.add_argument("--voxel_cache", default=None, help="복셀 캐시 폴더 또는 voxel_cache.zip 위치")
    ap.add_argument("--vessels", default=None, help="자항선 스펙 JSON (없으면 기본값)")
