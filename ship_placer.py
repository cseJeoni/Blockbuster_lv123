...
# -*- coding: utf-8 -*-
"""
Config 기반 자항선 블록 배치 시스템 (VoxelBlock3D 데이터 로딩 최종 완성 버전)
"""
import json
import sys
import os
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python ship_placer.py <config.json> [max_time] [options]")
        print("Options:")
    ...
    if result:
        ...
        if enable_visualization:
            output_dir = Path(placer_config.config_path).parent / "placement_results"
            output_dir.mkdir(exist_ok=True)
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            config_name = Path(placer_config.config_path).stem
            deadspace_suffix = "_deadspace" if show_dead_space else ""
            viz_filename = f"placement_{config_name}_{timestamp}{deadspace_suffix}.png"
