...
class GreedyPlacer:
    ...
    def optimize(self):
                ...
                for j, (pos_x, pos_y) in enumerate(candidates):
                    if self.placement_area.can_place_block(block, pos_x, pos_y):
                        ...
                        try:
                            ...
                            # 오른쪽으로 이동 최적화
                            self._move_block_right_to_fit(self.placement_area, block, spacing, bow_clearance)
                            
                            # 아래쪽으로 이동 최적화 
                            self._move_block_down_to_fit(self.placement_area, block, spacing)
        ...
        # 배치 못한 블록들의 이름 리스트만 결과에 포함
        unplaced_names = [block.id for block in unplaced_blocks]
        self.placement_area.unplaced_blocks = unplaced_names
