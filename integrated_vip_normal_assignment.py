...
class IntegratedVoyageAssigner:
        ...
        if os.path.exists(candidate):
            try:
                ...
                for entry in data.get("vessels", []):
                    specs[int(entry["id"])] = { "name": entry["name"], "width": int(entry["width"]), "height": int(entry["height"]) }
                if specs: return specs
            except Exception as e: print(f"[WARN] vessel_specs.json 로드 실패: {e}")
        return { 1: {"name": "자항선1", "width": 62, "height": 170}, 2: {"name": "자항선2", "width": 36, "height": 84}, 3: {"name": "자항선3", "width": 32, "height": 120}, 4: {"name": "자항선4", "width": 40, "height": 130}, 5: {"name": "자항선5", "width": 32, "height": 116} }
    ...
    def run_for_single_voyage(self, vessel_name: str, end_date: str, avail_vip: Set[str], avail_norm: Set[str], start_date: Optional[str] = None, cooldown_last_end: Optional[str] = None, cooldown_gap_days: int = 14) -> Dict:
        if cooldown_last_end:
            ...
            if d_end < d_min: return {"voyage_id": f"{vessel_name}_{end_date}", "placed_blocks": [], "vip_placed": [], "normal_placed": [], "path": "SKIPPED_COOLDOWN"}
        ...
        before_vip, before_norm = set(avail_vip), set(avail_norm)
    ...
    def save(self):
        ...
        # [해결] 비용 계산 로직 추가
        total_cost = 0
        cost_vessel_1 = 9.5e8  # 9.5억
        cost_vessel_other = 3.5e8 # 3.5억
        ...
        for vid in used_voyages:
            ...
            if vessel_name == "자항선1":
                total_cost += cost_vessel_1
            else:
                total_cost += cost_vessel_other
